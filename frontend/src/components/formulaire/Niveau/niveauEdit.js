...
const NiveauEdit = ({ item, onSubmit }) => {
  const [designniveau, setDesignniveau] = useState(item.designniveau);
  ...
  const handleSubmit = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const updatedItem = { ...item, designniveau};

        axios
          .patch(`http://localhost:5000/niveau/${item.id_niveau}`, updatedItem)
          .then(() => {
      ...
      <div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            ...
            <input type="text" className="form-control" value={designniveau} onChange={(e) => setDesignniveau(e.target.value)} />
          </div>

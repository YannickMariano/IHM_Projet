...
const FirebaseRegister = ({ ...others }) => {
  ...
  function submitRegistration() {
    if (mdp === confirmPassword) {
      axios.post("http://localhost:5000/prof/signup", {
        nom: nom,
        email: email,
        mdp: mdp
      }).then(function () {
        console.log("Creation de compte réussi !");
        setCurrentUser(true);
  ...
  return (
    ...
    {/* {successMessage && (
      ...
      validationSchema={Yup.object().shape({
        ...
        email: Yup.string().max(255).required('email requis'),
        mdp: Yup.string().max(255).required('mdp requis')
      })}

...
exports.signup = async (req, res) => {
    try {
        ...
        const { rows } = await Eleve.createUser(email, hash); // Utilise le hash
        if (rows.length > 0) {
...
exports.login = async (req, res) => {
    try {
        const { email, mdp } = req.body;
        const { rows } = await Eleve.getUser(email); // S'assurer que cette méthode est bien définie et fonctionnelle
        if (rows.length === 0) {
        ...
        const token = jwt.sign({ matricule: user.matriule, email: user.email }, 'RANDOM_TOKEN_SECRET', { expiresIn: '24h' });
        res.status(200).json({
